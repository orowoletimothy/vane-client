...
export default function DashboardPage() {
  ...
  const calculateTodayProgress = () => {
    ...
    const completed = todayHabits.filter(
      (habit: Habit) => habit.completedToday >= habit.target_count
    ).length;
          ...
          {weekDates.map((dayData) => (
            <button
              ...
              className={`flex flex-col items-center p-2 rounded-xl transition-colors ${
                dayData.isToday ? "bg-white shadow-md" : "hover:bg-white/50"
              }`}
            >
              <span className="text-xs text-gray-500 mb-1">{dayData.day}</span>
              <span
                className={`text-lg font-semibold ${
                  dayData.isToday ? "text-gray-900" : "text-gray-600"
                }`}
              >
                {dayData.date}
              </span>
          ...
          {todayHabits.length === 0 ? (
            ...
            filteredHabits.map((habit: Habit) => (
              <HabitCard
                ...
                habit={{
                  ...habit,
                  completedToday: habitProgress[habit.id] || 0,
                }}
